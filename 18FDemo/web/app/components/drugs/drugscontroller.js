...
drugscontrollers.DrugsController = function($scope, $http, $location, $routeParams) 
{
        ...
        $scope.indiSearch = function(term) {
            ...
            $http.get('https://api.fda.gov/drug/event.json?count=patient.reaction.reactionmeddrapt.exact&limit=1000&search=(patient.drug.openfda.generic_name:' + searchTerm + '+patient.drug.openfda.brand_name:' + searchTerm + ')'+filter)
                    .success(function(jsonResp) {
                        $scope.storage[searchTerm] = jsonResp.results;
                    }).error(function(errorResp) {
                        $scope.storage[searchTerm] = {};
                });
        };
        ...
        $scope.combinedSearch = function() {
            ...
            $scope.finalstorage = [];
            $http.get('https://api.fda.gov/drug/event.json?limit=10&count=patient.reaction.reactionmeddrapt.exact&search=(patient.drug.openfda.generic_name:' + combinedSearchTerm + '+patient.drug.openfda.brand_name:' + combinedSearchTerm + ')'+filter)
            .success(function(jsonResp) {
                $scope.headerItems = [];
